  ...
  return (
    <div className="flex gap-2 justify-center">
      <Button
        ...
        onClick={() => googleLogin()}
        disabled={!googleClientId || loading !== null}
        title={googleClientId ? 'Sign in with Google' : 'Set VITE_GOOGLE_CLIENT_ID in .env to enable'}
      >
        {loading === 'google' ? <Loader2 className="size-5 animate-spin" /> : <GoogleIcon />}
      </Button>
    </div>
  );
}
