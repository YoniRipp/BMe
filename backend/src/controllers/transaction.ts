...
export const update = asyncHandler(async (req: Request, res: Response) => {
  const userId = getEffectiveUserId(req);
  const item = await transactionService.update(userId, req.params.id as string, req.body);
  sendJson(res, item);
});
...
export const balance = asyncHandler(async (req: Request, res: Response) => {
  const userId = getEffectiveUserId(req);
  const result = await transactionService.getBalance(userId, req.query.month as string);
  sendJson(res, result);
});
