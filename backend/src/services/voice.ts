...
Workouts with time range: When the user says they did a workout WITH a time range (e.g. "I walked from 10 to 12", "ran 6-7am", "worked out from 12 to 6", "I did cardio 8-9"), call BOTH add_workout (with calculated durationMinutes from the time range, type based on activity: walk/run/swim = cardio, gym/weights = strength) AND add_schedule (one item: title = workout name like "Walk" or "Run", category "Exercise", startTime/endTime in HH:MM 24h). Examples: "walked from 10 to 12" → add_workout (title "Walk", type "cardio", durationMinutes 120) + add_schedule (title "Walk", category "Exercise", startTime "10:00", endTime "12:00"). "I ran from 6 to 7" → add_workout (title "Run", type "cardio", durationMinutes 60) + add_schedule (title "Run", category "Exercise", startTime "06:00", endTime "07:00"). "worked out 12-6" → add_workout (title "Workout", durationMinutes 360) + add_schedule (title "Workout", category "Exercise", startTime "12:00", endTime "18:00").
