...
describe('voice service HANDLERS', () => {
  ...
  describe('add_transaction', () => {
    it('builds merge with type, amount, category, date', async () => {
      const result = await HANDLERS.add_transaction(
        { type: 'expense', amount: 50, category: 'Food', description: 'Coffee' },
        { todayStr }
      );
    ...
    it('defaults to expense and today when type/date missing', async () => {
      const result = await HANDLERS.add_transaction({ amount: 100 }, { todayStr });
    ...
    it('defaults short item description to Food category', async () => {
      const result = await HANDLERS.add_transaction(
        { type: 'expense', amount: 5, description: 'Coke' },
        { todayStr }
      );

      expect(result.merge).toMatchObject({ category: 'Food' });
    });
  });

  describe('add_schedule', () => {
    it('builds items with title, startTime, endTime, category', async () => {
      const result = await HANDLERS.add_schedule(
        {
          items: [
            {
              title: 'Meeting',
