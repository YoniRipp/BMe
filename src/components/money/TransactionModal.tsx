...
import { validateTransactionAmount, validateTransactionDate, validateCategory } from '@/lib/validation';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function TransactionModal({ open, onOpenChange, onSave, transaction }: TransactionModalProps) {
  ...
  const validateField = (field: string, value: string) => {
    ...
    switch (field) {
      case 'amount':
        ...
        result = validateTransactionAmount(parseFloat(value));
        setErrors(prev => ({
          ...prev,
          amount: result.isValid ? '' : (result.error || '')
        }));
        break;
      case 'date':
        if (!value) {
        ...
        result = validateTransactionDate(new Date(value));
        setErrors(prev => ({
          ...prev,
          date: result.isValid ? '' : (result.error || '')
        }));
        break;
      case 'category':
        result = validateCategory(value, categories);
        setErrors(prev => ({
          ...prev,
          category: result.isValid ? '' : (result.error || '')
        }));
        break;
    }
  };
